...
interface Booking {
  ...
  returnAllocatedDriver: number | null; // NEW: Separate driver for return journey
  bookingDate: string;
  flightNumber?: string;
  driverNotes?: string;
}
...
const DUMMY_BOOKINGS: Booking[] = [
  {
   id: Date.now() + 1,
    ...
    status: "Allocated",
    allocatedDriver: 1,
    returnAllocatedDriver: 2, // Different driver for return
...
const Transfer = () => {
  ...
  const [isReturnJourney, setIsReturnJourney] = useState(false); // NEW: Track if allocating for return journey
  ...
  const handleAddBooking = () => {
    ...
    const booking: Booking = {
     id: Date.now() + Math.floor(Math.random() * 1000),
      ...
      returnAllocatedDriver: null, // NEW
  ...
  // UPDATED: Allocate driver with separate handling for outward/return
 // UPDATED: Allocate driver with separate handling for outward/return
const allocateDriver = () => {
  ...
  console.log('🔵 ALLOCATING DRIVER:', {
    bookingId: selectedBookingForAllocation.id,
  ...
  setBookings(prevBookings => {
    const updated = prevBookings.map(booking => {
      // Only update THIS specific booking by ID
      if (booking.id === selectedBookingForAllocation.id) {
        console.log('✅ MATCHED BOOKING:', booking.id);
        if (isReturnJourney) {
          // Allocating for RETURN journey
          return { 
            ...booking, 
            returnAllocatedDriver: selectedDriver.id
          };
        } else {
          // Allocating for OUTWARD journey
          return { 
            ...booking, 
            allocatedDriver: selectedDriver.id, 
            status: "Allocated" 
          };
        }
      }
      console.log('⏭️ SKIPPING BOOKING:', booking.id);
      return booking; // Don't modify other bookings
    });
    console.log('📦 NEW BOOKINGS STATE:', updated);
    return updated;
  });
  ...
  // UPDATED: Remove driver with separate handling
 // UPDATED: Remove driver with separate handling
const removeDriver = (bookingId: number, isReturn: boolean = false) => {
  console.log('🔴 REMOVING DRIVER:', { bookingId, isReturn });
  
  setBookings(prevBookings => {
    const updated = prevBookings.map(booking => {
      // Only update THIS specific booking by ID
      if (booking.id === bookingId) {
        console.log('✅ MATCHED BOOKING FOR REMOVAL:', booking.id);
        if (isReturn) {
          return { ...booking, returnAllocatedDriver: null };
        } else {
          return { ...booking, allocatedDriver: null, status: "Pending" };
        }
      }
      console.log('⏭️ SKIPPING BOOKING:', booking.id);
      return booking; // Don't modify other bookings
    });
    console.log('📦 NEW BOOKINGS STATE AFTER REMOVAL:', updated);
    return updated;
  });
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ position: 'sticky', top: 0, backgroundColor: '#f8f9fa', zIndex: 10 }}>
              <tr style={{ backgroundColor: '#f8f9fa', borderBottom: '2px solid #dee2e6' }}>
                ...
                <th style={{ padding: '12px', textAlign: 'left', fontSize: '13px', fontWeight: '600', color: '#495057' }}>{t("dashboard.Pickup Date")}</th>
              ...
              {bookings.map((booking, index) => (
                <React.Fragment key={booking.id}>
                  <tr style={{ borderBottom: '1px solid #dee2e6' }}>
                    ...
                    {/* UPDATED: Driver allocation for OUTWARD journey only */}
                   <td style={{ padding: '12px' }}>
  {booking.allocatedDriver ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          <button
                            onClick={() => removeDriver(booking.id, false)}
                            style={{
                              background: 'none',
                              border: 'none',
                              cursor: 'pointer',
                  ...
                  {/* UPDATED: Expanded return row with separate driver allocation */}
                          ...
                          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tbody>
                              <tr>
                                ...
                                {/* UPDATED: Separate driver for RETURN journey */}
                                <td style={{ padding: '10px' }}>
                                  {booking.returnAllocatedDriver ? (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                      <span style={{ fontSize: '13px', color: '#333' }}>
                                        {drivers.find(d => d.id === booking.returnAllocatedDriver)?.name}
                                      </span>
                                      <button
                                        onClick={() => removeDriver(booking.id, true)}
                                        style={{
                                          background: 'none',
                                          border: 'none',
                                          cursor: 'pointer',
...
{allocateDialogVisible && (
  <div
    ...
    onClick={() => { setAllocateDialogVisible(false); setSelectedDriver(null); setIsReturnJourney(false); }}
  >
    <div
    ...
    >
      <div className="modal-header">
        <h2>{t("dashboard.Allocate Driver")} {isReturnJourney && "(Return Journey)"}</h2>
        <button
          className="close-btn"
          onClick={() => { setAllocateDialogVisible(false); setSelectedDriver(null); setIsReturnJourney(false); }}
        >
      ...
      <div className="modal-body">
        <div className="modal-booking-info">
          ...
          <p className="gray">
            {isReturnJourney 
              ? `${selectedBookingForAllocation?.toLocation} → ${selectedBookingForAllocation?.fromLocation}`
        ...
        <div className="modal-actions">
          <button className="btn-outline" onClick={() => { setAllocateDialogVisible(false); setSelectedDriver(null); setIsReturnJourney(false); }}>
            {t("dashboard.Cancel")}
          </button>
...
{addBookingDialogVisible && (
  <div className="modal-overlay" onClick={() => { setAddBookingDialogVisible(false); setFormErrors({}); }}>
    <div className="modal-box large-modal" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="modal-header" style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 100 }}>
      ...
      <div className="modal-body scrollable-box">
        <div style={{ padding: '24px' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '16px', color: '#2196F3' }}>
              {t("dashboard.Customer Information")}
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', fontWeight: '500' }}>
                  {t("dashboard.Customer Name")} *
                </label>
                <input 
                  type="text" 
                  ...
                  style={{ 
                    width: '100%', 
                    padding: '10px', 
                    border: formErrors.customerName ? '1px solid #f44336' : '1px solid #d1d5db', 
                    borderRadius: '6px', 
                    fontSize: '14px' 
                  }} 
                />
                ...
                <div>
                  ...
                  <input 
                    ...
                    style={{ 
                      width: '100%', 
                      padding: '10px', 
                      border: formErrors.customerEmail ? '1px solid #f44336' : '1px solid #d1d5db', 
                      borderRadius: '6px', 
                      fontSize: '14px' 
                    }} 
                  />
                ...
                <div>
                  ...
                  <input 
                    ...
                    style={{ 
                      width: '100%', 
                      padding: '10px', 
                      border: formErrors.customerPhone ? '1px solid #f44336' : '1px solid #d1d5db', 
                      borderRadius: '6px', 
                      fontSize: '14px' 
                    }} 
                  />
              ...
              <div>
                ...
                <input 
                  ...
                  style={{ 
                    width: '100%', 
                    padding: '10px', 
                    border: formErrors.fromLocation ? '1px solid #f44336' : '1px solid #d1d5db', 
                    borderRadius: '6px', 
                    fontSize: '14px' 
                  }} 
                />
              ...
              <div>
                ...
                <input 
                  ...
                  style={{ 
                    width: '100%', 
                    padding: '10px', 
                    border: formErrors.toLocation ? '1px solid #f44336' : '1px solid #d1d5db', 
                    borderRadius: '6px', 
                    fontSize: '14px' 
                  }} 
                />
            ...
            <div>
              ...
              <select 
                ...
                style={{ 
                  width: '100%', 
                  padding: '10px', 
                  border: formErrors.carType ? '1px solid #f44336' : '1px solid #d1d5db', 
                  borderRadius: '6px', 
                  fontSize: '14px', 
          ...
          {/* PICKUP DETAILS - FIXED Clock Icon */}
          <div style={{ marginBottom: '24px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '16px', color: '#2196F3' }}>
              {t("dashboard.Pickup Details")}
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px', fontSize: '14px', fontWeight: '500' }}>
                ...
                <input 
                  ...
                  style={{ 
                    width: '100%', 
                    padding: '10px', 
                    border: formErrors.pickupDate ? '1px solid #f44336' : '1px solid #d1d5db', 
                    borderRadius: '6px', 
                    fontSize: '14px' 
                  }} 
                />
              ...
              <div>
                ...
                <input 
                  ...
                  style={{ 
                    width: '100%', 
                    padding: '10px', 
                    border: formErrors.pickupTime ? '1px solid #f44336' : '1px solid #d1d5db', 
                    borderRadius: '6px', 
                    fontSize: '14px' 
                  }} 
                />
          ...
          {/* Add Return Journey Checkbox */}
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
              <input 
                type="checkbox" 
                checked={newBooking.hasReturn} 
                onChange={(e) => handleInputChange('hasReturn', e.target.checked)}
                style={{ width: '18px', height: '18px', cursor: 'pointer' }} 
              />
              <span style={{ fontSize: '14px', fontWeight: '500' }}>Add Return Journey</span>
            </label>
          </div>
          {/* RETURN DETAILS - FIXED with date validation */}
          {newBooking.hasReturn && (
            <div style={{ marginBottom: '24px', padding: '16px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '16px', color: '#2196F3' }}>
                {t("dashboard.Return Details")}
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px', fontSize: '14px', fontWeight: '500' }}>
                  ...
                  <input 
                    ...
                    min={newBooking.pickupDate || getTodayDate()}
                    style={{ 
                      width: '100%', 
                      padding: '10px', 
                      border: formErrors.returnDate ? '1px solid #f44336' : '1px solid #d1d5db', 
                      borderRadius: '6px', 
                      fontSize: '14px' 
                    }} 
                  />
                ...
                <div>
                  ...
                  <input 
                    ...
                    style={{ 
                      width: '100%', 
                      padding: '10px', 
                      border: formErrors.returnTime ? '1px solid #f44336' : '1px solid #d1d5db', 
                      borderRadius: '6px', 
                      fontSize: '14px' 
                    }} 
                  />
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '24px' }}>
            <button 
              onClick={() => { setAddBookingDialogVisible(false); setFormErrors({}); }}
              style={{ 
                flex: 1, 
                padding: '12px', 
                backgroundColor: '#fff', 
  ...
  >
    ...
    >
      ...
      <div className="sidebar-content">
        ...
        {/* Driver */}
        <div className="section-divider">
          <h3 className="section-title">Assigned Driver (Outward)</h3>
          <div className="details-text">
        ...
        {selectedBooking.returnDate && (
          <div className="section-divider">
            <h3 className="section-title">Assigned Driver (Return)</h3>
            <div className="details-text">
              {selectedBooking.returnAllocatedDriver ? (
                <>
                  <div><strong>Driver:</strong> {drivers.find(d => d.id === selectedBooking.returnAllocatedDriver)?.name}</div>
                  <div><strong>Phone:</strong> {drivers.find(d => d.id === selectedBooking.returnAllocatedDriver)?.phone}</div>
                </>
              ) : (
                <div className="no-driver">No driver assigned for return journey yet</div>
              )}
            </div>
          </div>
        )}
